...
module.exports = function createRequire(filename) {
    var fakeRequire = requireLike(filename);
    return function requireMocked(path, opts) {
        return new MockedLoader(fakeRequire.resolve(path), opts).load();
    };
};
